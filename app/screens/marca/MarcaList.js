...
export default function MarcaList() {
  ...
  const listGet = async () => {
    try {
      const response = await axios.get(
        "http://192.168.1.3:8080/api/marca/lista"
      );
  ...
  const deleteItem = async (id) => {
    try {
      await axios.delete(`http://192.168.1.3:8080/api/marca?id=${id}`);
  ...
  const updateItem = (id) => {
    navigation.navigate("marcaatualizar", { id });
  };
  ...
  return (
    <ScrollView>
      <Pressable
        style={styles.button}
        onPress={() => navigation.navigate("marcacadastro")}
      >
        <Text style={styles.text}>Adicionar uma marca</Text>
      </Pressable>
      {data.map((item) => (
